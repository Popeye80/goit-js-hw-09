...
formEl.addEventListener("submit", function (event) {
  ...
  var delay = Number(dataFormObject.delay);
  var amount = Number(dataFormObject.amount);
  var step = Number(dataFormObject.step);
  var stepNumber;
  var iStep;
  ...
  var _loop = function _loop(iDelay) {
    setTimeout(function () {
      var iiDelay;
      ...
      position += 1;
      createPromise(position, iiDelay).then(function (_ref) {
  ...
  for (var iDelay = delay; iDelay < stepNumber; iDelay += iStep) {
    _loop(iDelay);
  }
});
